            ...
            try:
                if (processed_data.breathing_result):
                    if (processed_data.breathing_result.breathing_rate):
                        ...
                        print(processed_data.breathing_result.breathing_rate)
                        results = np.float64(processed_data.breathing_result.breathing_rate)
                        tosend = np.array([currentTime, results])
                        #Sends an array of the time and respiration rate to NumPy file
                        nf.write(tosend)
                    else:
