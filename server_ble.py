...
# true if there's been an overdose alert, false otherwise.
alert = False
    ...
    while True:
        try:
            ...
            if data:
                ...
                print(f"{IAM} received: {data_decoded}")
                if data_decoded == "Web Server Connecting.":
                    await characteristic.write(encode_message(f"{MESSAGE_FOR_DISPENSER}"))
                    await asyncio.sleep(0.5)
                else:
                    await characteristic.write(encode_message(f"{MESSAGE}"))
                    await asyncio.sleep(0.5)
